...
- 交付：`app_wx_user` 表、`src/modules/app/**`（auth / catalog / member）、`AppAccessTokenGuard`、
  独立 Swagger 分组，以及 **7 个**写接口的契约骨架（返回 501）
  （`POST /app/auth/phone` 原本也在骨架清单里，施工单 A8 已把它换成真实现；
  `GET /app/member/cards` 由 A9 换成真实现。两者都不计入 501；见 §16.1）
- **验收**：
  ...
  - **7 个**骨架接口在 Swagger 可见、schema 完整，调用返回 501 且不落库
...
| 真实现   | `POST /app/auth/login`、`POST /app/auth/phone`（A8 转真实现）、`GET /app/service-items`、`GET /app/staffs`、`GET /app/available-slots`、`GET /app/member/me`、`GET /app/member/cards`（A9 转真实现） |
| 契约骨架 | 以下 **7 个**返回 501（不落库）：`GET / POST /app/bookings`、`POST /app/bookings/:id/cancel`、`POST /app/reviews`、`POST /app/payments/wxpay/jsapi`、`POST /app/subscribe`、`POST /app/payments/wxpay/notify` |
...
| 小程序端评价                 | `POST /app/reviews` 骨架已留（复用 `biz_review` 的一单一评约束）              |
