...
**「当前门店」的来源与优先级**（阶段 1.7 起，后台顶栏的门店切换器）：

| 顺序 | 来源                                              | 列表里                            | 写入时             |
| ---- | ------------------------------------------------- | --------------------------------- | ------------------ |
| 1    | 显式传参（列表 `?storeId=`、写入 body `storeId`） | 按它筛选；不可见 → 403            | 落它；不可见 → 403 |
| 2    | 请求头 `x-store-id`（切换器选择）                 | 按它筛选                          | 落它               |
| 3    | ——（都没传）                                      | 可见范围内全部（超管 = 全店合并） | 默认门店           |

第 2 级**不可见时静默忽略**（授权可能刚被收回，这不是用户的显式请求，不该报 403 刷屏）；
`GET /stores/mine` 负责把选项与「选中的门店」一起回给前端，本地残留的失效门店在那里被归一回
「全部门店」。切换器只在**可见门店 > 1** 时显示，单店期看不到这个概念。
