...
1. **门店切换器**（web 顶栏）：超管看各店数据目前要在 URL 上手加 `?storeId=`
   （预约、收款、退款、应收四个列表都已支持）。
2. **报表按店 + 合并**：报表口径（净营收、营业日）还是全局；应按门店拆分并支持合并视图。
3. **顾客档案 / 会员资产是否按店**：现在是**共享**（通兑假设）。若要「按店独立」，见阶段 2。
4. **小程序选店**：`GET /app/shops`（列表）+ 下单带 `storeId`；现在小程序全程用默认门店。
5. **支付多商户号**：现在是单商户号（`WXPAY_*` / `ALIPAY_*` 各一套）。加盟/独立结算才需要多套。
