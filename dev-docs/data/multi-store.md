...
**阶段 0 / 1 / 1.5 / 1.6 / 1.7 / 1.8 / 1.9 已完成**：门店是实体、单据带门店、按账号的门店范围过滤与筛选、
门店管理页与用户门店授权、**顶栏门店切换器（列表筛选与写入落店一起走）**、
**报表按店（营收 / 单量 / 项目 / 美甲师 / 应收 / 次卡核销）**、
**美甲师可服务门店（列表 / 选人 / 小程序目录都按店收窄）**都已生效；
**排班按店、小程序选店、支付多商户号尚未做**（见「还没做」）。
...
1. **排班按门店**：现在只做到「人能服务哪些店」，班次仍是**一人一份周模板** ——
   ...
   **可约时段（`slots.service`）按门店取班次** + 排班周视图加门店维度 + 请假冲突按店判 +
   小程序「选店 → 选人 → 选时段」链路。这是「真正按门店运营」的最后一块。
2. **会员资产按店**：报表里「储值充退 / 结存 / 积分 / 新增会员 / 次卡发售」仍是**全店口径**，
   ...
   即文末「阶段 2」的前置。
3. **小程序选店**：`GET /app/shops`（列表）+ 下单带 `storeId`；现在小程序全程用默认门店。
4. **支付多商户号**：现在是单商户号（`WXPAY_*` / `ALIPAY_*` 各一套）。加盟 / 独立结算才需要多套。
5. **门店维度的其它视图**：工作台、统计卡这类非列表视图目前不跟着切换器走
