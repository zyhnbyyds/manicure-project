...
**阶段 0 / 1 / 1.5 / 1.6 / 1.7 / 1.8 / 1.9 / 1.10 / 1.11 已完成**：门店是实体、单据带门店、按账号的门店范围过滤与筛选、
门店管理页与用户门店授权、**顶栏门店切换器（列表筛选与写入落店一起走）**、
**报表按店（营收 / 单量 / 项目 / 美甲师 / 应收 / 次卡核销）**、
**美甲师可服务门店（列表 / 选人 / 小程序目录都按店收窄）**、
**小程序选店与定位最近门店**、**按门店排班（专属班次优先、通用兜底）**都已生效；
**会员资产按店、支付多商户号尚未做**（见「还没做」）。
...
1. **会员资产按店**：报表里「储值充退 / 结存 / 积分 / 新增会员 / 次卡发售」仍是**全店口径**，
   ...
   即文末「阶段 2」的前置。
2. **支付多商户号**：现在是单商户号（`WXPAY_*` / `ALIPAY_*` 各一套）。加盟 / 独立结算才需要多套。
3. **门店维度的其它视图**：工作台/统计卡这类非列表视图目前不跟着切换器走
   （`useTable` / 收银台队列 / 报表页 / 排班页已接）。新增业务视图时记得判一次 `activeStoreId`；
