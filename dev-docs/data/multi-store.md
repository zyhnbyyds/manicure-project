...
**阶段 0 / 1 / 1.5 / 1.6 / 1.7 已完成**：门店是实体、单据带门店、按账号的门店范围过滤与筛选、
门店管理页与用户门店授权、**顶栏门店切换器（列表筛选与写入落店一起走）**都已生效；
**报表按店、小程序选店、支付多商户号尚未做**（见「还没做」）。
...
1. **报表按店 + 合并**：报表口径（净营收、营业日）还是全局 —— 也就是说**切换门店后报表数字不变**，
   预约/收款/退款/应收列表已经跟手切换器，报表还没接。应按门店拆分并支持合并视图。
2. **顾客档案 / 会员资产是否按店**：现在是**共享**（通兑假设）。若要「按店独立」，见阶段 2。
...
5. **门店维度的其它视图**：工作台、统计卡这类非列表视图目前不跟着切换器走
   （`useTable` 与收银台队列已接）。新增业务视图时记得判一次 `activeStoreId`。
