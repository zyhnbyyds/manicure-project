...
1. **排班按门店**：现在只做到「人能服务哪些店」，班次仍是**一人一份周模板** ——
   所以「同一个人在 A 店排周一、在 B 店排周三」表达不出来。要做需：
   `biz_staff_weekly_shift` / `biz_staff_schedule_override` 加 `store_id` +
   **可约时段（`slots.service`）按门店取班次** + 排班周视图加门店维度 + 请假冲突按店判。
   这是「真正按门店运营」的最后一块（小程序选店已就绪，可以直接接）。
2. **会员资产按店**：报表里「储值充退 / 结存 / 积分 / 新增会员 / 次卡发售」仍是**全店口径**，
