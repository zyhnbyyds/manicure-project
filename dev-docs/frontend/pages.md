...
| 14  | `biz_refunds`            | `/biz/refunds`            | `web/src/views/biz/refunds/index.vue`            | 退款管理   | `biz:refund:list`          | `biz:refund:apply`；`biz:refund:approve`（服务中退款 / 重试执行 / 驳回）                                                  |
...
| --- | ------------------------ | ------------------------- | ------------------------------------------------ | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | `dashboard`              | `/dashboard`              | `web/src/views/dashboard/index.vue`              | 首页（**经营概览**，按店） | 首页菜单本身无权限点；页面内容按 `biz:report:view`（全量·含金额）/ `biz:report:home`（轻量·不含金额，挂在 `dashboard_home_overview` 按钮行）二选一渲染 |
