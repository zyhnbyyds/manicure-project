...
| 5   | 改退款判责档位 ⚠️              | `src/modules/biz/payment/refunds/refunds.service.ts`（`matchPolicy` 约 724 行、`assess` 约 683 行）、`web/src/views/biz/refunds/index.vue`                                            | 匹配规则：`status=active` 且 `hours_before <= 实际小时数`，按 `hours_before DESC, sort ASC, id ASC` 取第一条；`liable != customer` 时全退                                 | `src/database/seed/biz.ts` 的 `REFUND_POLICY_SEEDS`（0 小时档位必须保留，覆盖已过开始时间）            |
