...
function parseArgs() {
  ...
  if (!['stable', 'latest'].includes(options.channel)) {
    throw new Error(
      `不支持的 channel：${options.channel}，仅支持 stable、latest 或 nightly`,
    );
  }
...
function isRedirectDownloadUrl(url) {
  return (
    url.origin === 'https://servicewechat.com' &&
...
function isAllowedDownloadUrl(value) {
  try {
    ...
    return (
      isRedirectDownloadUrl(url) ||
      isReleaseDownloadUrl(url) ||
      isNightlyDownloadUrl(url)
    );
  } catch {
    return false;
  }
...
function getMatchedType(url, preferredTypes) {
  ...
  return (
    preferredTypes.find((type) => url.pathname.includes(`_${type}.`)) || null
  );
}
...
async function selectDownload(urls, preferredTypes, options) {
  ...
  const candidates = buildCandidates(urls, preferredTypes, requirePkg)
    .filter(
      (candidate) =>
        options.channel !== 'latest' || candidate.kind === 'nightly',
    )
    .sort(sortCandidates);
  ...
  throw new Error(`未找到可用的 macOS PKG（已跳过：${rejected.join('；')}）`);
}
...
async function main() {
  ...
  const scriptResults = await Promise.allSettled(
    scriptUrls.map((url) => getText(url)),
  );
  ...
  const fallbackReason =
    options.channel === 'stable' && selected.kind !== 'stable'
