...
function ensureDarwin(install) {
  ...
  if (legacyTarget && fs.existsSync(legacyTarget)) {
    legacyLink = createUnixSymlink(
      path.join(binDir, 'wechatidecli'),
      legacyTarget,
    );
  }
  ...
  return result(true, {
    action:
      link.created || legacyLink?.created || pathInfo.pathUpdated
        ? 'linked'
        : 'already_linked',
    command: 'wechatide',
    ...
    pathHint:
      pathInfo.pathUpdated || pathInfo.alreadyConfigured
...
function readWindowsUserPath() {
  ...
  if (result.error || result.status !== 0) {
    throw new Error(
      result.error?.message || (result.stderr || '读取用户 PATH 失败').trim(),
    );
  }
...
function writeWindowsUserPath(newPath) {
  ...
  if (result.error || result.status !== 0) {
    throw new Error(
      result.error?.message || (result.stderr || '写入用户 PATH 失败').trim(),
    );
  }
}
...
function ensureWin32(install) {
  ...
  return result(true, {
    ...
    pathUpdated: !already,
    pathHint:
      '新开终端后 wechatide 生效；当前会话可临时把安装目录加入 PATH，或直接用 targetPath',
    ...
    console.log(
      JSON.stringify(
        result(false, {
          ...
          error:
            '安装目录存在，但缺少 wechatide 入口（勿用 wechatidecli / cli 冒充）',
