...
export const MIN_COMPATIBLE_VERSION_NUMBER = versionToNumber(
...
function extractWindowsRegistryPaths(output) {
  const paths = [];
  const valuePattern =
    /^\s+(?:InstallLocation|DisplayIcon|UninstallString|\(Default\))\s+REG_\w+\s+(.+)$/i;
...
function findRuntimeRoot(platform, candidate) {
  ...
  for (let depth = 0; depth < 5; depth += 1) {
    ...
    if (
      fs.existsSync(packagePaths.nw) ||
      fs.existsSync(packagePaths.electron)
    ) {
      return current;
    }
...
export function getInstallRoots(platform, installRoot) {
  ...
  if (platform === 'win32') {
    ...
    const commonRoots = programFilesRoots.map((root) =>
...
export function resolveInstallRoot(platform, installRoot) {
  ...
  const resolvedRoot =
    checkedInstallRoots.find((root) => fs.existsSync(root)) || null;
