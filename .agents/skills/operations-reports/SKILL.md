...
- **门店维度**（阶段 1.8）：接口接 `?storeId=`，不传则用顶栏切换器的 `x-store-id` 头；
  **店长不传任何参数也只统计本店**（数据权限）。但**储值充退 / 期末结存 / 积分 / 新增会员 /
  次卡发售恒为全店口径** —— 资产表没有门店列（余额是全店通兑的池子），前端必须标「全店」，
  否则店长会以为「切到本店后这些数字也是本店的」。详见 `dev-docs/backend/reports.md` 的「门店维度」。
