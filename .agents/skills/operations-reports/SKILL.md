...
- **门店维度**（阶段 1.8 起，1.12 扩到会员流水）：接口接 `?storeId=`，不传则用顶栏切换器的
  `x-store-id` 头；**店长不传任何参数也只统计本店**（数据权限）。
- **两条口径必须分清，混了就是算错账**（阶段 1.12）：
  - **按店（发生额）**：营收 / 退款 / 单量 / 客单价 / 项目 / 美甲师 / 应收账龄 / 次卡核销，
    以及**储值充值 / 储值赠送 / 积分发放 / 积分抵扣**（这四项 1.12 起按**流水发生门店**统计，
    前端「全店」标签已撤掉）；
  - **恒为全店（存量与主体）**：**期末结存 / 新增会员 / 次卡发售** —— 余额是全店通兑的一个池子、
    顾客入会没有门店概念、`biz_member_card` 没有门店列，按店切分只会得出「A 店余额」这种不存在的概念。
    前端必须标「全店」，否则店长会以为「切到本店后这些数字也是本店的」。
- **判据一句话**：这个数字回答的是「**某家店发生了什么**」（按店）还是「**这家公司现在有多少**」（全店）。
- 详见 `dev-docs/data/multi-store.md` 的「阶段 1.12」与 `dev-docs/backend/reports.md` 的「门店维度」。
