 ...
 *
 * **这个 IP 会随 DHCP 变**（2026-09 就变过四次：`.101` → `.100` → `172.17.64.1` → `10.167.225.38` →
 * `172.18.0.1`，是老地址连不通的头号原因），
 * 连不上时先按下面这条命令核对，别怀疑代码：
 * ```
 * Get-NetIPAddress -AddressFamily IPv4 |
 *   Where-Object { $_.IPAddress -notlike '127.*' -and $_.PrefixOrigin -eq 'Dhcp' }
 * ```
 * **判据是「物理网卡」（`InterfaceAlias` = WLAN / 以太网），不是「有默认网关」** ——
 * 这条曾经写错过，代价是排查了半天防火墙：Hyper-V 的 `vEthernet (Default Switch)`
 * （本机 `172.18.0.1`）**也有默认网关**，且**本机 `curl` 它一样返回 200**，
 * 但手机根本不在那个虚拟网段里 → 真机一律超时。同理要避开 VMware `VMnet1/8`、蓝牙、`本地连接*`。
 * 本机当前是 WLAN `192.168.0.101`。
