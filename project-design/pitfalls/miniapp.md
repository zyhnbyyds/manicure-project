...
## 22. 盖在 `<swiper>` 上的浮层会**吃掉触摸**：那一片区域划不动

- **现象**：首页头图改成轮播后，压在图上的「今日灵感」文案与右下角圆点那一片**划不动**，
  只有图的上半部分能翻页；而 `pointer-events: none` 写上去毫无效果。
- **根因**：`<swiper>` 的滑动由它自己的触摸监听实现；压在它上面的兄弟节点（浮层 `view`）
  会成为触摸目标，事件**不会**再冒泡回 swiper。`pointer-events` 在 WXSS 里不可靠
  （WebView 渲染下常被忽略），指望它放行手势是错的。
- **正确做法**：**渐变与文案放进 `<swiper-item>` 里**（每屏各带一份，跟着一起滑），
  只有尺寸很小的自绘圆点留在外面。这样整屏（含文字区）都能正常翻页。
- **同类坑**：设计稿要「右下角圆点」时**不能用原生 `indicator-dots`**（只能居中）——
  自绘圆点 + `bindchange` 记下标，`circular`/`autoplay` 下自动轮播也会正常触发 `bindchange`。
- **来源**：首页按新设计稿（`manicure-ui-home`）改成轮播时踩到。

## 23. 卡片里的心形用 `bindtap` = 点收藏却跳进了详情页

- **现象**：点「人气款式」卡右上角的收藏心，收藏状态闪了一下，但页面同时跳进了款式详情
  （体感是「收藏没生效」）。
- **根因**：心形是「整卡可点进详情」那张卡的子节点，`bindtap` 默认**冒泡**，
  于是父级的 `openService` 也被触发。
- **正确做法**：卡片内部的一切独立动作（收藏、去支付、取消…）一律 **`catchtap`**（阻止冒泡）。
  这也是本项目既有页面（我的预约 / 收银台单卡）一致的写法。
- **附带一条**：收藏态**以服务端返回的目标状态为准**（`{ favorited }`），不要在本地取反 ——
  双击 / 慢网 / 并发点两次时本地取反必然与真实状态错位；未绑定手机号时**先门禁再发请求**
  （接口会 400 + needBind，明知失败还打一次只会让「未登录」看起来像「加载失败」）。
- **来源**：首页卡上新增收藏按钮时。

## 24. 改了不生效 / 一直报 `X is not defined`：开发者工具把**中间态**编译进去了

- **现象**：首页报 `<ReferenceError: PAGE_ICONS is not defined>`，栈指向
  `appservice-hotreload/pages/index/index.js:107`；但源码里**根本没有** `PAGE_ICONS`
  （`tsc --noEmit -p tsconfig.json` 也是 exit 0）。
- **根因**：改动分了两步落地 —— 先删掉 `const PAGE_ICONS = [...]`，几秒后才把
  `chromeIcons: PAGE_ICONS` 换成 `whiteIcons: QUICK_ICONS`。**中间那几秒文件是
  「有引用、无声明」的坏状态**，开发者工具的编译/热重载正好抓到它并留在内存里；
  之后即使源码改回来，热重载也**不会**重建那个模块作用域，于是错误一直复现。
- **正确做法**：
  1. **删/改顶层常量时声明与引用一次改完**（一次写入，别分两次编辑）；
  2. 已经踩上就「**工具 → 清除缓存 → 清除全部缓存**」再编译，或直接重启开发者工具；
  3. 报错**先别改代码**，核实「源码是否真的还有那个符号」：全仓搜一遍符号名，再
     `tsc -p tsconfig.json --outDir <临时目录> --rootDir ./miniprogram` 看**产物**里有没有它。
     产物干净 = 工具缓存问题，改代码只会越改越乱。
- **一句话**：`tsc` 绿 ≠ 运行时不报 —— 前者查源码，后者查工具手里那份 bundle。
- **来源**：首页改成新设计稿后由用户反馈；从报错行号反推出「跑的是中间态」。
