...
## 17. 让人手填内部 ID（顾客 ID / 预约 ID / 用户 ID）

- **现象**：「纳为会员」弹窗里只有一个「顾客 ID」数字框，「发起退款申请」里只有「预约 ID」，
  次卡与收款列表的筛选框也是「顾客 ID」文本框。运营 / 店长**既看不到也猜不出**这些值，
  只能先去别的页面翻出 ID、抄下来、再回来粘 —— 一步能做完的事变成三步。
- **根因**：把**数据库主键当成了输入控件**。主键是给程序用的，不是给操作员看的；
  凡是「引用另一个实体」的字段，都该是**选择**而不是**录入**。
- **正确做法**：一律「关键词输入 + 搜索按钮 + 下拉选择」，选项里带**人能认的字段**：

  | 要选的实体 | 用什么                                                                | 选项文案                                        |
  | ---------- | --------------------------------------------------------------------- | ----------------------------------------------- |
  | 顾客       | `useCustomerOptions()`（`web/src/composables/useCustomerOptions.ts`） | `姓名 / 手机号`（+ `无手机号` / `已入会` 标记） |
  | 预约       | `listBookings(1, 30, { keyword })`                                    | `单号 · 顾客 · 到店时间`                        |
  - 打开弹窗时**预载一批**（30~200 条），多数场景不用搜；从行内打开时用
    `ensure(customer)` 把已知实体塞进选项，否则下拉是空白。
  - 前置条件要在选项上体现：顾客没手机号就入不了会（后端 400），
    所以 `search(kw, size, { requirePhone: true })` 会把这类选项**置灰**，
    从详情打开时干脆**提示并拒绝打开弹窗**。

- **`LewSelect` 没有 `search` 事件**（只有 `change` / `blur` / `clear` / `focus` / `delete`），
  所以远程搜索**只能是**「输入框 + 搜索按钮 + 下拉」；`searchable` 只是**本地**过滤，
  选项没载进来就搜不到。
- **例外（刻意保留）**：`staffs` 的「后台账号」在没有「用户管理」列表权限时退化为手填用户 ID ——
  拿不到候选列表，只能如此，`tips` 里已写明原因。
- **来源**：用户报「会员管理纳为会员为什么要输入顾客 ID???」（2026-09-14）。
  顺带全量扫了一遍 `field: 'xxxId'`：其余要么本来就是 `select`，要么是表格只读列。

---

## 18. 确认框里的实体名字存成了独立变量 —— 改选之后显示旧名字

- **现象**：从会员详情点「纳为会员」打开弹窗（预置了当前顾客「张三」），
  用户改选成「李四」再提交，二次确认框上仍写着**张三** —— 确认文案与实际提交的对象不一致。
- **根因**：打开弹窗时把「详情里那位顾客的名字」存进了一个独立 ref
  （`enrollCustomerName`），并在确认文案里 `独立变量 || 当前选中项`。
  默认路径（不改选）完全正确，所以**自测很容易漏**。
- **正确做法**：实体名字**只从当前选中项对应的 options 里取**：

  ```ts
  const picked = options.value.find((item) => item.value === String(id));
  const who = picked?.label ?? `#${id}`;
  ```
