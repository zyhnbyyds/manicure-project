...
**同类坑反复踩是本项目最大的时间浪费**。所以这里有一条硬规则：

### 规则

1. **动手前先扫**：改哪一端，就先读对应的坑文件（尤其在同一模块反复改动时）；
2. **踩到就立刻写**：一个新坑解决之后，**在同一个提交里**追加一条
   —— 不要攒着，事后一定想不起细节；
3. **只写会再犯的坑**：一次性的笔误、拼写错误不用写；
4. **格式固定**：`现象 → 根因 → 正确做法 → 怎么发现的`，
   便于扫读；能给出「决定性诊断手段」的最好一并写上
   （例：渲染异常先量元素尺寸，别靠截图猜）。
...
本会话的真实数字：PowerShell 转义/锚点/`cd` 这几类坑**各自重复踩了 3~4 次**，
每次平均十几分钟；集成测试从 ~530 秒优化到 ~18 秒之前，
每天要为此等待若干次。**写下来一次，后面所有人省掉。**
