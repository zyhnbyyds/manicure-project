 ...
 *
 * 使用步骤：
 *   1. 将代码放到 /usr/apps/manicure-project（git clone 或 rsync）
 ...
 *   4. 启动并保存：
 *        mkdir -p /usr/apps/manicure-project/logs
 *        pm2 start ecosystem.config.js
 *        pm2 save          # 配合 pm2 startup 实现开机自启
 *   5. 常用命令：
 *        pm2 status / pm2 logs manicure-server / pm2 restart manicure-server
 *   6. 记得在防火墙/安全组放行 TCP 1011；如需域名可再配一层反向代理
 */
module.exports = {
  apps: [
    {
      name: 'manicure-server',
      cwd: '/usr/apps/manicure-project',
      ...
      time: true, // 日志行带时间戳
      merge_logs: true,
      out_file: '/usr/apps/manicure-project/logs/out.log',
      error_file: '/usr/apps/manicure-project/logs/error.log',
